...
final class SupportAccountContext {
    ...
    func open(account: Account) {
        ...
        _ = data.start(next: { folders in
            ...
            let applicationContext = AuthorizedApplicationContext(window: window, context: context, launchSettings: LaunchSettings.defaultSettings, callSession: nil, groupCallContext: nil, folders: folders)
